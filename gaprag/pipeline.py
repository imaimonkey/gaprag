...
class GapRAGPipeline:
    ...
    def run_query(
        ...
        retrieval_conf = float(np.mean(retrieval_probs)) if retrieval_probs.size else 0.0
        ...
        if memory_type == "none":
            memory_vec = gap_vec
            memory_stats = {"type": "none", "updated": False}
        else:
            memory = self._get_session_memory(run_mode, session_id)
            previous = memory.get(query_key=query_hidden)
            memory.update(gap_vec, query_key=query_hidden)
            current = memory.get(query_key=query_hidden)
            if current is None:
                current = gap_vec
            memory_vec = current
            memory_stats = {
                "type": memory_type,
                "updated": True,
                "prev_norm": float(np.linalg.norm(previous)) if previous is not None else 0.0,
        ...
        return PipelineOutput(
            ...
            gap_stats={
                ...
                "avg_retrieval_prob": retrieval_conf,
            },
