...
def run_setting(
    ...
    for step_idx, item in enumerate(tqdm(dataset, desc=f"{setting}/{mode}"), start=1):
        ...
        parsed_prediction = canonicalize_prediction(parsed_prediction, item.get("answers", []))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run stateless vs continual evaluation for GapVerify")
